"""
generate_from_plantilla.py
Genera el informe oficial de laboratorio a partir de SI886-PLANTILLA-TALLER (1).docx
conservando fielmente los estilos, encabezados, tablas y estructura oficial de la UPT/EPIS.
"""
...
def run():
    ...
    doc.paragraphs[11].runs[0].font.size = Pt(13)
    ...
    doc.paragraphs[14].runs[0].font.size = Pt(11)
    doc.paragraphs[14].runs[0].font.bold = True

    doc.paragraphs[17].text = (
        "CALIZAYA FLORES, KEVIN ALEXANDER · 2021070123\n"
        "MAMANI CHOQUE, RODRIGO ALONSO · 2021070456\n"
        "QUISPE TICONA, MARCO ANTONIO · 2021070789"
    )
    doc.paragraphs[17].runs[0].font.name = "Calibri"
    doc.paragraphs[17].runs[0].font.size = Pt(10)
    ...
    # -------------------------------------------------------------
    # 2. ELIMINAR SECCIÓN "CÓMO SE USA ESTA PLANTILLA" (P50 - P57)
    # Según la plantilla oficial: "Borra esta página antes de entregar."
    # -------------------------------------------------------------
    paragraphs_to_remove = []
    found_start = False
    for p in doc.paragraphs:
        if "Cómo se usa esta plantilla" in p.text:
            found_start = True
        if found_start:
            paragraphs_to_remove.append(p)
            if "Borra esta página antes de entregar" in p.text:
                break
    ...
    for p in doc.paragraphs:
        txt = p.text.strip()
        if "\t" in txt:
            continue # saltar entradas del TOC
    ...
    print("Found headings:", list(p_dict.keys()))
    ...
    recursos_data = [
        ...
        ("Git & GitHub", "2.x", "Control de versiones, trazabilidad y creación de etiquetas oficiales 'v0.5' y 'taller-05'."),
    ...
    cur = insert_p_after(cur, "Diseño del instrumento contextualizado '02_identidad/CU01_instrumento.md' y levantamiento de una muestra de N=35 encuestas distribuidas en seis gerencias representativas de la sede central de la MPT, registradas en '02_identidad/encuesta_cultura.csv'.", bold_prefix="• Comando o acción: ")
    cur = insert_p_after(cur, "Archivo de especificación del instrumento '02_identidad/CU01_instrumento.md' con las 6 dimensiones adaptadas a la corporación edil y dataset estructurado '02_identidad/encuesta_cultura.csv' conteniendo 35 registros y 51 variables de evaluación.", bold_prefix="• Evidencia de que funcionó: ")
    ...
    cur = insert_p_after(cur, "Desarrollo y ejecución del script automatizado en Python: 'python 02_identidad/CU02_perfil_cultura.py'.", bold_prefix="• Comando o acción: ")
    cur = insert_p_after(cur, "El script auditó las 35 encuestas, identificando y excluyendo con éxito 3 casos inconsistentes (R015 en OGAF con 110 pts; R024 en OGACGD con 90 pts; R031 en GTPSC con 105 pts), procesando 32 encuestas válidas (91.4% de efectividad). Se generó la matriz '02_identidad/CU02_perfil_cultura.csv', el informe de control de calidad 'docs/evidencias/S05/salidas/control_calidad.txt' y el renderizado '02_identidad/CU_perfil_cultura.png'.", bold_prefix="• Evidencia de que funcionó: ")
    ...
    cur = insert_p_after(cur, "Análisis hermenéutico y de discurso sobre el ROF 2022, RISST 2024 (51 pp.), PEI 2025-2030 y Ley 27815, sintetizado en '02_identidad/CU03_supuestos.csv'.", bold_prefix="• Comando o acción: ")
    ...
    cur = insert_p_after(cur, "Construcción de la matriz estratégica '02_identidad/CU04_implicancias_peti.md', articulándola con los objetivos de Gobierno Digital del PEI 2025-2030 (OEI.07).", bold_prefix="• Comando o acción: ")
    ...
    cur = insert_p_after(cur, "Redacción de '02_identidad/2.3_valores.md' y '02_identidad/2.4_cultura.md', incorporando criterios de decisión tecnológica.", bold_prefix="• Comando o acción: ")
    ...
    cur = insert_p_after(cur, "Respuesta argumentada y ejecución de comandos: 'git commit -m \"...\"', 'git tag -a v0.5' y 'git tag -a taller-05'.", bold_prefix="• Comando o acción: ")
    cur = insert_p_after(cur, (
        ...
        "Evidencia de versionamiento: Commits registrados y etiquetas 'v0.5' y 'taller-05' verificables en 'docs/evidencias/S05/salidas/git_log.txt'."
    ), bold_prefix="• Evidencia de que funcionó: ")
    ...
    chk_14 = [
        ("1", "Instrumento CVF con 6 dimensiones y 2 momentos", "LOGRADO", "02_identidad/CU01_instrumento.md"),
        ("2", "Al menos 10 respondientes de diversas áreas", "LOGRADO", "N=35 en 02_identidad/encuesta_cultura.csv"),
        ("3", "Control de calidad ejecutado (exclusión sumas != 100)", "LOGRADO", "salidas/control_calidad.txt (3 excluidos)"),
        ("4", "Perfil de cultura actual y deseada con brechas", "LOGRADO", "02_identidad/CU02_perfil_cultura.csv"),
        ...
        ("7", "Perfil por área funcional (áreas con n >= 5)", "LOGRADO", "OGTIC, GGT, OGAF, OGACGD, GTPSC analizadas"),
        ("8", "Gráfico de radar generado en alta resolución", "LOGRADO", "02_identidad/CU_perfil_cultura.png"),
        ("9", "Al menos 5 supuestos básicos inferidos con evidencia", "LOGRADO", "6 supuestos en 02_identidad/CU03_supuestos.csv"),
        ("10", "Al menos 3 brechas entre valor adoptado y supuesto", "LOGRADO", "5 brechas demostradas en CU03_supuestos.csv"),
        ("11", "Tabla de implicancias con estrategia de implantación", "LOGRADO", "02_identidad/CU04_implicancias_peti.md"),
        ("12", "Valores formulados como conductas con consecuencias", "LOGRADO", "5 valores conductuales en 02_identidad/2.3_valores.md"),
        ("13", "Valor que gobierna decisión tecnológica concreta", "LOGRADO", "4 tensiones y criterios TI en Sección 2.3.2"),
        ("14", "Secciones 2.3 y 2.4 redactadas y etiqueta v0.5", "LOGRADO", "Etiquetas v0.5 y taller-05 creadas en Git")
    ]
    ...
    t1.cell(0, 1).width = Inches(3.2)
    t1.cell(0, 2).width = Inches(1.0)
    t1.cell(0, 3).width = Inches(2.4)
    ...
    for item in chk_14:
        ...
        format_row(new_row, "F8FAFC" if len(t1.rows)%2==1 else "FFFFFF", font_size=8)
    ...
    for p in doc.paragraphs:
        if "Si algo no se logró, explícalo aquí" in p.text:
            p.text = (
                "Evaluación global de resultados: Todos los catorce resultados previstos en la lista de comprobación de la Semana 05 fueron alcanzados satisfactoriamente sin observaciones pendientes. Las 3 encuestas que violaron la regla de suma 100 fueron descartadas y registradas con fines de auditoría metodológica, garantizando la pureza del diagnóstico cultural."
            )
            p.style = "Normal"
            p.paragraph_format.space_after = Pt(8)
    ...
    for p in doc.paragraphs:
        if "Capturas completas, archivos de configuración y salidas extensas" in p.text:
            p.text = (
                "Anexo A: Instrumento de Diagnóstico CVF (OCAI) · Municipalidad Provincial de Tacna\n"
                "Archivo: anexos/anexo_A_instrumento_cvf.pdf (y .docx)\n"
                "Descripción: Cuestionario completo de seis dimensiones, con las cuatro afirmaciones contextualizadas a la corporación edil, regla de suma 100 e instrucciones metodológicas.\n\n"
                "Anexo B: Matriz Consolidada de Resultados de Cultura Organizacional\n"
                "Archivo: anexos/anexo_B_resultados_cultura.xlsx\n"
                "Descripción: Libro en formato Microsoft Excel con las 32 encuestas conformes depuradas y las hojas de promedios, brechas dimensionales y datos del radar.\n\n"
                "Anexo C: Gráfico de Radar del Perfil Cultural (Alta Resolución)\n"
                "Archivo: anexos/anexo_C_perfil_cultura.png\n"
                "Descripción: Imagen renderizada a 300 DPI que ilustra el contraste polar entre la cultura Actual (Jerarquía dominante) y la cultura Deseada al 2030 (Adhocracia).\n\n"
                "Anexo D: Matriz de Supuestos Básicos e Inferencias de la MPT\n"
                "Archivo: anexos/anexo_D_supuestos_basicos.xlsx\n"
                "Descripción: Matriz en Excel estructurada con los seis supuestos básicos inferidos a partir del ROF 2022, RISST 2024 y PEI 2025-2030, detallando las brechas observadas.\n\n"
                "Anexo E: Secciones Oficiales 2.3 y 2.4 del PETI de la MPT\n"
                "Archivo: anexos/anexo_E_secciones_2_3_2_4.pdf (y .docx)\n"
                "Descripción: Texto normativo final aprobado de la Sección 2.3 (Valores institucionales y tecnológicos) y Sección 2.4 (Diagnóstico cultural), versionado bajo la etiqueta v0.5."
            )
            p.style = "Normal"
            p.paragraph_format.space_after = Pt(8)

    doc.save(output_docx)
    print(f"Documento Word oficial generado con éxito a partir de la plantilla: {output_docx}")
